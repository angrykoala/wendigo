        ...
        return new Promise((resolve, reject) => {
            ...
            this._events.once("on-request", () => {
                if (!rejected) {
                    ...
                    setTimeout(() => {
                        resolve();
                    }, 20); // Give time to browser to handle the response
                }
            });
        });
    }
    ...
    _assertCalled(times, msg) {
        if (typeof times === 'number') {
            if (times !== this.timesCalled) {
                ...
                throw new AssertionError("assert.called", msg, this.timesCalled, times);
            }
        } else if (!this.called) {
            if (!msg) msg = `Mock of ${this.url} not called.`;
            throw new AssertionError("assert.called", msg);
        }
    }
