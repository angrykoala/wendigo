...
const {QueryError, FatalError} = require('../../errors');
...
module.exports = class BrowserAssertions {
    ...
    text(selector, expected, msg) {
        if ((!expected && expected !== "") || (Array.isArray(expected) && expected.length === 0)) return Promise.reject(new Error(`Missing expected text for assertion`));
    ...
    async url(expected, msg) {
        ...
        try {
            ...
            throw new FatalError(`Can't obtain page url.${err.message}`);
        }
