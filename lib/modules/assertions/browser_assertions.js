...
module.exports = class BrowserAssertions {
    ...
    style(selector, style, expected, msg) {
        return this._browser.evaluate((sel, sty) => {
            ...
            const error = new QueryError("assert.style", `Element "${selector}" not found when trying to assert style.`);
    ...
    focus(selector, msg) {
        return this._browser.evaluate((q) => {
            ...
            const error = new QueryError("assert.focus", `Element "${selector}" not found when trying to assert focus.`);
            return Promise.reject(error);
        }).then((focused) => {
            if (!focused) {
