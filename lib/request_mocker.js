    ...
    _assertCalled(times) {
        if(typeof times === 'number') {
            if(times !== this._timesCalled) {
                throw ErrorFactory.generateAssertionError(`Mock of ${this.url} not called ${times} times.`, this._timesCalled, times);
            }
        } else if(!this.called) {
            throw ErrorFactory.generateAssertionError(`Mock of ${this.url} not called.`);
        }
    }
