...
module.exports = function BrowserQueriesMixin(s) {
    return class BrowserQueries extends s {
        ...
        findCssPath(domElement) {
            if (!(domElement instanceof DomElement)) return Promise.reject(new Error("Invalid element for css path query."));
        ...
        findXPath(domElement) {
            if (!(domElement instanceof DomElement)) return Promise.reject(new Error("Invalid element for xPath query."));
            else return this.evaluate((e) => {
        ...
        elementFromPoint(x, y) {
            ...
            return this.page.evaluateHandle((evalX, evalY) => {
                ...
                else return DomElement._processQueryResult(elementHandle);
            });
        }
