...
module.exports = function BrowserActionsMixin(s) {
    return class BrowserActions extends s {
        ...
        /**
         * @param {string|number} selector A query selector, or the X value for x/y coordinates.
         * @param {number} index The nth element to choose based on 'selector', or, the Y value for x/y coordinates.
         * @return {Promise}
         */
        click(selector, index) {
            ...
            if (typeof selector === 'number' && typeof index === 'number') {
                return this.evaluate((evalX, evalY) => {
                    document.elementFromPoint(evalX, evalY).click();
                }, selector, index);
            }
