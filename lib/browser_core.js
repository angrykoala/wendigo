...
module.exports = class BrowserCore {
    ...
    open(url, options) {
        ...
        return this._beforeOpen(options).then(() => {
            ...
            if (err.message === "Evaluation failed: Event") return Promise.reject(new InjectScriptError(err)); // CSP error
            else if (err instanceof FatalError) return Promise.reject(err);
    ...
    addScript(scriptPath) {
        ...
        return this.page.addScriptTag({
            ...
            return Promise.reject(new InjectScriptError(err));
        });
    }
