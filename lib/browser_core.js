...
module.exports = class BrowserCore {
    ...
    open(url, options) {
        ...
        return this._beforeOpen(options).then(() => {
            ...
            return Promise.reject(ErrorFactory.generateFatalError(`Failed to open ${url}. ${err.message}`));
        });
    }
    ...
    openFile(filepath, options) {
        ...
        return this.open(`file:${absolutePath}`, options).catch(() => {
            return Promise.reject(ErrorFactory.generateFatalError(`Failed to open "${filepath}". File not found.`));
        });
    }
    ...
    close() {
        ...
        return this._beforeClose().then(() => {
            return this.page.close().catch((err) => {
                return Promise.reject(ErrorFactory.generateFatalError(`Failed to close browser. ${err.message}`));
            });
        });
    }
